...
: Node("kinect_ros2", options)
{
  ...
  bool lock_auto_exposure       = this->declare_parameter("lock_auto_exposure",       false);
  bool lock_auto_white_balance  = this->declare_parameter("lock_auto_white_balance",  false);
  ...
  if (lock_auto_exposure) {
    freenect_set_flag(fn_dev_, FREENECT_AUTO_EXPOSURE, FREENECT_OFF);
    RCLCPP_INFO(this->get_logger(), "Kinect AE disabled");
  }
  if (lock_auto_white_balance) {
    freenect_set_flag(fn_dev_, FREENECT_AUTO_WHITE_BALANCE, FREENECT_OFF);
    RCLCPP_INFO(this->get_logger(), "Kinect AWB disabled");
  }
